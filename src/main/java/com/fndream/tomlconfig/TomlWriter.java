...
import com.fndream.tomlconfig.TomlConfig.MetaValue;
...
class TomlWriter {
    ...
    TomlWriter writeMap(Map<String, MetaValue> map, String path) {
        if (inLineArray) {
            sb.append("{ ");
            int i = 0;
            int size = map.size();
            ...
            for (Map.Entry<String, MetaValue> entry : map.entrySet()) {
                MetaValue metaValue = entry.getValue();
    ...
    TomlWriter writeList(List<?> list, String path) {
        if (!inLineArray && isTableArray(list)) {
            ...
            for (int i = 0; i < list.size(); i++) {
                writeValue(new MetaValue(list.get(i)), path);
                if (i < list.size() - 1) {
                    sb.append(", ");
                }
            }
    ...
    private void writeValue(MetaValue metaValue, String path) {
        Object value = metaValue.value();
    ...
    private Map<String, MetaValue> toMetaMap(Object obj) {
        if (obj instanceof TomlConfig tomlConfig) {
            ...
            Map<String, MetaValue> arrayMap = new LinkedHashMap<>();
            Map<String, MetaValue> tableMap = new LinkedHashMap<>();
            for (Map.Entry<String, Object> entry : tomlTable.toMap().entrySet()) {
                String key = entry.getKey();
                Object value = entry.getValue();
                if (value instanceof TomlArray) {
                    arrayMap.put(key, new MetaValue(value));
                } else if (value instanceof TomlTable) {
                    tableMap.put(key, new MetaValue(value));
                } else {
                    resultMap.put(key, new MetaValue(value));
                }
            }
            ...
            for (Map.Entry<?, ?> entry : map.entrySet()) {
                ...
                Object value = entry.getValue();
                if (value instanceof TomlArray) {
                    arrayMap.put(key, new MetaValue(value, null));
                } else if (value instanceof TomlTable) {
                    tableMap.put(key, new MetaValue(value, null));
                } else {
                    resultMap.put(key, new MetaValue(value, null));
                }
            }
            resultMap.putAll(arrayMap);
            resultMap.putAll(tableMap);
